...
namespace Utils
{
    ...
    class Helper
    {
        ...
        public static string  GetAttdTableName(string _ip,string constr)
        {

            string retstr = string.Empty;
            string selerr = string.Empty;
            DataSet ds = Utils.Helper.GetData("Select * from ReaderConfig Where MachineIP ='" + _ip + "'", constr, out selerr);
            bool hasrows = ds.Tables.Cast<DataTable>().Any(table => table.Rows.Count != 0);
            if (hasrows)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                   string _machinedesc = dr["MachineDesc"].ToString();
                   bool _messflg = Convert.ToBoolean(dr["CanteenFlg"]);
                   bool _lunchinout = Convert.ToBoolean(dr["LunchInOut"]);
                   bool _gateinout = Convert.ToBoolean(dr["GateInOut"]);
                   
                    if (_lunchinout)
                        retstr = "AttdLunchGate";
                    else if (_messflg)
                        retstr =  "AttdLog";
                    else if (_gateinout)
                        retstr = "AttdGateInOut";
                    else if (_machinedesc.Contains("Water Vikaspuram"))
                        retstr = "AttdWater";
                    else
                        retstr = "AttdLog";
