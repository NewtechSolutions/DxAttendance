...
{
    public partial class frmMastEmpMisConduct : Form
    {
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        ...
                        sql = string.Format(sql,txtMisConDt.DateTime.ToString("yyyy-MM-dd") ,txtMisConDesc.Text.Trim().ToString(),
                            ctrlEmp1.txtEmpUnqID.Text.Trim(),ctrlEmp1.txtEmpUnqID.Text.Trim(),ctrlEmp1.txtWrkGrpCode.Text.Trim(),
