...
{
    public partial class frmMastEmpBulkAddress : DevExpress.XtraEditors.XtraForm
    {
        ...
        private string DataValidate(DataRow tdr)
        {
            ...
            if(tdr["AddressType"] == DBNull.Value)
            {
                err = "AddressType is required..('PER','PER')" + Environment.NewLine;
                return err;
            }

            string tAddType = tdr["AddressType"].ToString().ToUpper();

            if (!"PER,PRE".Contains(tAddType))
            {
                
                err += "Invalid AddressType , use any of..('PER','PER')" + Environment.NewLine;
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(Utils.Helper.constr))
                {
                    ...
                    foreach (DataRow dr in sortedDT.Rows)
                    {
                        ...
                        string AddType = dr["AddressType"].ToString().ToUpper();
                        bool tfound = false;
                        string tsql = "Select count(*) from MastEmpAddress where EmpUnqID ='" + tEmpUnqID + "' and AddressType='" + AddType + "'";
                        ...
                        string tAdd1 = dr["Area"].ToString().Trim();
                        string tAdd2 = dr["Society"].ToString().Trim();
                        string tAdd3 = dr["Village"].ToString().Trim();                       
                        string tAdd4 = dr["Taluka"].ToString().Trim();
                        string tCity = dr["City"].ToString().Trim();
                        string tState = dr["State"].ToString().Trim();
                        string tDistrict = dr["District"].ToString().Trim();
                        string tPin = dr["PinCode"].ToString().Trim();
                        string tPhone = dr["Phone"].ToString().Trim();
                        ...
                        using (SqlCommand cmd = new SqlCommand())
                        {
                            try
                            {
                                ...
                                if (tfound)
                                {
                                   sql= "Update MastEmpAddress " +
                                        " Set Add1='" + tAdd1 + "', Add2='" + tAdd2 + "'," + 
                                        " Add3='" + tAdd3 + "', Add4='" + tAdd4 + "'," +
                                        " City='" + tCity + "', StateName='" + tState + "'," +
                                        " District='" + tDistrict + "'," +
                                        " UpdDt =GetDate() , UpdID='" + Utils.User.GUserID + "'" + 
                                        " PinCode='" + tPin + "', Phone='" + tPhone + "' Where EmpUnqID ='" + tEmpUnqID + "' And AddressType='" + AddType + "'";
                                }
                                else
                                {
                                    sql = "insert into MastEmpAddress (EmpUnqID,AddressType,Add1,Add2,Add3,Add4,City,District,StateName,Pincode,Phone,AddDt,AddID) Values (" +
                                        "'" + tEmpUnqID + "','" + AddType + "','" + tAdd1 + "','" + tAdd2 + "','" + tAdd3 + "','" + tAdd4 + "'," +
                                        "'" + tCity + "','" + tDistrict + "','" + tState + "','" + tPin + "','" + tPhone + "',GetDate(),'" + Utils.User.GUserID + "')";
                                }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string myexceldataquery = "select EmpUnqID,AddressType,Area,Society,Village,Taluka,City,District,State,PinCode,Phone, " +
        ...
        private void frmMastEmpBulkAddress_Load(object sender, EventArgs e)
        {
