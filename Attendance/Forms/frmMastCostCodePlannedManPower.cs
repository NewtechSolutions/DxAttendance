...
{
    public partial class frmMastCostCodePlannedManPower : Form
    {
        public string mode = "NEW";
        public string GRights = "XXXV";
        public string oldCode = "";

        public frmMastCostCodePlannedManPower()
        {
            InitializeComponent();
        }

        private void frmMastCostCodePlannedManPower_Load(object sender, EventArgs e)
        {
            ResetCtrl();
        ...
        private string DataValidate()
        {
            ...
            string sql = "Select max(ValidFrom) From MastCostCodePlannedManPower where CostCode = '" + txtCostCode.Text.Trim().ToString() + "' " +
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Insert into MastCostCodePlannedManPower (CostCode,ValidFrom,CompEmp,ContEmp,AddDt,AddID) "
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Update MastCostCodePlannedManPower set "
            ...
            if (string.IsNullOrEmpty(err))
            {
                ...
                using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        {
                            cn.Open();
                            string sql = "Delete From MastCostCodePlannedManPower " 
        ...
        {
            DataSet ds = new DataSet();
            string sql = "Select CostCode,ValidFrom,CompEmp,ContEmp FromMastCostCodePlannedManPower where " 
        ...
        private void txtValidFrom_EditValueChanged(object sender, EventArgs e)
        {
            ...
            DataSet ds = new DataSet();
            string sql = "select * From MastCostCodePlannedManPower where CostCode ='" + txtCostCode.Text.Trim().ToString() + "' " +
                    " and ValidFrom ='" + txtValidFrom.DateTime.ToString("yyyy-MM-dd") + "' ";
        ...
        private void frmMastCostCodePlannedManPower_KeyDown(object sender, KeyEventArgs e)
        {
