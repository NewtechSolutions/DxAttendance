...
namespace Attendance.Forms
{
    public partial class frmTranLeaveEntry : Form
    {
        ...
        private void frmSanction_Load(object sender, EventArgs e)
        {
            ...
            MeFormID = Convert.ToInt32("0" + Utils.Helper.GetDescription("Select FormID from MastFrm Where FormName = 'frmTranLeaveEntry'", Utils.Helper.constr));
        ...
        private void LoadLeaveBalGrid()
        {
            ...
            string sql = "Select a.LeaveTyp,a.Opn,a.Avl,(a.OPN-(a.AVL+a.ADV+a.ENC)) as Bal ,a.Adv,a.ENC from leaveBal a,MastLeave b " +
             " where  a.LeaveTyp = b.LeaveTyp and b.WrkGrp ='" + Emp.WrkGrp + "' " +            
        ...
        private void LoadLeaveDetails()
        {
            ...
            if(txtLeaveTyp.Text.Trim() == "AB" || txtLeaveTyp.Text.Trim() == "LWOP" || txtLeaveTyp.Text.Trim() == "SUL" || txtLeaveTyp.Text.Trim() == "RH")
            {
                WODayNo = 0;
                HLDay = 0;
            }
           ...
           "     ) ";
            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                ...
                if (!hasRows)
                {
                    try
                    {
                        string insertsql = "insert into LeaveEntry (CompCode,tYear,EmpUnqID,FromDt,ToDt," +
                            " LeaveTyp,TotDay,WoDay,PublicHL,LeaveDed,LeaveADV,LeaveHalf,Remark,AddID,AddDt,DelFlg) " +
                            " Values ('" + Emp.CompCode + "','" + FromDt.Year.ToString() + "','" + Emp.EmpUnqID + "','" + FromDt.ToString("yyyy-MM-dd") + "','" + ToDt.ToString("yyyy-MM-dd") + "', " +
                    ...
                    foreach (DataRow trslr in trsLV.Tables[0].Rows)
                    {
                        ...
                        if(WoEntReq == false)
                        {
                            ...
                            string sqldel = "Update MastLeaveSchedule Set SchLeave = null, SchLeaveHalf = 0,SchLeaveAdv = 0 Where " +
                                " EmpUnqID='" + Emp.EmpUnqID + "' " +
                                " And tDate ='" + CalDt.ToString("yyyy-MM-dd") + "'" +
                                " And SchLeave in ('WO','PH') and WrkGrp ='" + Emp.WrkGrp + "' ";
                    ...
                    using (SqlCommand cmd20 = new SqlCommand())
                    {
                        try
                        {
                            string tsql1 = "Update LeaveBal " +
                                ...
                                " Where tYear ='" + FromDt.Year.ToString() + "'" +
                                " And EmpUnqID ='" + Emp.EmpUnqID + "'" +
                                " And LeaveTyp='" + LeaveTyp + "'";
