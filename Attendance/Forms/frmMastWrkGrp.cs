            ...
            using (SqlConnection cn = new SqlConnection(Utils.Helper.constr))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        cn.Open();
                        cmd.Connection = cn;
                        string sql = "Update MastWrkGrp Set WrkGrp = '{0}',WrkGrpDesc = '{1}', UpdDt = GetDate(), UpdID = '{2}' Where CompCode = '{3}' and WrkGrp = '{4}' ";

                        sql = string.Format(sql, txtWrkGrpCode.Text.Trim().ToString(), txtDescription.Text.Trim(),
                             Utils.User.GUserID, txtCompCode.Text.Trim().ToString(), oldWrkGrp
                           );
