    ...
    public partial class frmLunchHalfDaypost : Form
    {
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txtEmpUnqID.Text.Trim()))
            {
                 ...
                 sql = "Select CompCode,WrkGrp,EmpUnqID,OTFlg from mastemp where CompCode = '01' and Wrkgrp = '" + sWrkGrp + "' And Active = 1  Order By EmpUnqID";
                ...
                sql = "Select CompCode,WrkGrp,EmpUnqID,OTFlg From MastEmp Where CompCode ='" + txtCompCode.Text.Trim() + "' "
                    + " and WrkGrp = '" + sWrkGrp + "' "
                            ...
                            {  
                                ...
                                foreach (DataRow trs in AttdLunchDs.Tables[0].Rows)
                                {
                                    ...
                                    if ( (trs["LeaveStatus"].ToString()) != "" && (trs["LeaveStatus"].ToString()) == "AB")
                                    {
                                        ...
                                        sql = "Select Count(*) from LeaveEntry where CompCode = '" + trs["CompCode"] + "' and " +
                                        " WrkGrp = '" + trs["WrkGrp"] + "' and tYear = '" + Convert.ToDateTime(trs["tDate"]).ToString("yyyy") + "' " +
                                        " And EmpUnqID = '" + trs["EmpUnqID"].ToString() + "' and FromDt ='" + Convert.ToDateTime(trs["tDate"]).ToString("yyyy-MM-dd") + "' and isnull(LeaveTyp,'') <> '' ";
                                        ...
                                        else
                                        {
                                            ...
                                            cmd.Parameters.AddWithValue("@pFromDt", Convert.ToDateTime(trs["tDate"]));
                                            cmd.Parameters.AddWithValue("@pToDt", Convert.ToDateTime(trs["tDate"]));
                                            cmd.Parameters.AddWithValue("@pEmpUnqID", drs["EmpUnqID"].ToString());
                                ...
                                if (posted == true)
                                {
                                    ...
                                    //pending need for main proccess function to developed..........?????
